...
There are three ways to do the calibration.  The default ``balanced`` method sends a set of circuits
...
balanced bitstrings over four qubits are:
...
results to get the needed :math:`2N` error rates.  These two states are a subset of the ``balanced``
...
An example setting the method is as follows:
...
The ``rep_delay`` keyword argument sets the time between calibration circuits on IBM Quantum&reg; backends.
This option exists to test and reduce the presence of state prep errors.  All the calibration circuits
in M3 contain conditional resets at the beginning so as to minimize the need for setting this
option.
...
`initial_reset` is a boolean value that specifies whether reset instructions should be used at the beginning of the
...
actual circuits might still suffer from these errors.
...
bitstrings and the available free memory on the computer.  To override this, you
...
``distance``
~~~~~~~~~~~~
Optionally you can truncate the M3 assignment matrix to only those elements of the matrix that
...
By default, M3 computes the corrections out to the full distance.  In practice, if you include only
up to ``distance=3`` elements, the result will be accurate answers in most cases.
...
With `details`, you can see additional information about the solution.  This changes the return of the
...
`max_iter` sets the maximum number of iterations performed by the iterative solver.
...
`tol` sets the tolerance of the iterative solver.  You might need adjustments to ``max_iter`` if the value
is set too low.
